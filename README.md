...
- **网页模式**：内置交互式 FastAPI 页面，可在浏览器查看状态并发送文本。
- **SoundDevice 音频**：使用 `sounddevice` 库进行音频输入输出，避免依赖 PyAudio。
- **可选摄像头功能**：摄像头相关功能需要安装 OpenCV (`cv2`)，若未安装将自动跳过。
...
### Web 模式用法
使用网页模式启动后，在浏览器访问 `http://localhost:8000` 查看状态：
